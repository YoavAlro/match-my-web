...
describe("Azure OpenAI requests", () => {
  ...
  it("keeps deck refinements executable when the provider returns only a delta", async () => {
    ...
    const basePatch = {
      ...
      colorScheme: "unchanged" as const, contrast: "unchanged" as const, reduceMotion: false, strongFocus: false, hideSponsoredContent: false, hideVideoPosts: false, feedFilterTerms: [], hideSelectors: [],
    };
